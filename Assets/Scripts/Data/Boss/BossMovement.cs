...
namespace DreamLU
{
    public class BossMovement : MonoBehaviour
    {
        ...
        private void MoveEnemy()
        {
            ...
            if (!chasePlayer && Vector3.Distance(transform.position, playerPosition) < boss.Data.stat.chaseDistance)
            {
                chasePlayer = true;
            }
        ...
        private IEnumerator MoveEnemyRoutine(Stack<Vector3> movementSteps)
        {
            while (movementSteps.Count > 0)
            {
                ...
                while (Vector3.Distance(nextPosition, transform.position) > 0.2f)
                {
                    ...
                    boss.SetAnimationMovement(transform.position, nextPosition);
                    MoveRigidBody(nextPosition, transform.position, boss.MoveSpeed);
